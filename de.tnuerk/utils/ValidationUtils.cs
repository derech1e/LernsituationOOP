...
namespace LernsituationOOP.de.tnuerk.utils
{
    ...
    {
        public static bool IsEmailValid(string email) => Regex.IsMatch(email, "^([0-9a-zA-Z]([-\\.\\w]*[0-9a-zA-Z])*@([0-9a-zA-Z][-\\w]*[0-9a-zA-Z]\\.)+[a-zA-Z]{2,9})$");

        public static bool IsStringANumber(string number) => Regex.IsMatch(number, "^\\d+$");

        public static bool IsStringNotNull(string input) => !string.IsNullOrEmpty(input) || !string.IsNullOrWhiteSpace(input);
